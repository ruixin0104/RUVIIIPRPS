...
findNcgPerBiologyPerBatch <- function(
    ...
    if (assess.se.obj) {
        se.obj <- checkSeObj(
            ...
            variables = unique(c(
                bio.variables, uv.variables, bio.groups, uv.groups
            )),
            remove.na = remove.na,
            verbose = verbose
        )
    ...
    if (!is.null(regress.out.uv.variables) & !is.null(normalization)) {
        printColoredMessage(
            message = '-- Regress out UV variables:',
            color = 'magenta',
            verbose = verbose)
        printColoredMessage(
            message = paste0(paste0(regress.out.uv.variables, collapse = ' & '),
    ...
    if (!is.null(regress.out.bio.variables)) {
        printColoredMessage(
            message = '-- Regress out biological variables:',
            color = 'magenta',
            verbose = verbose)
        printColoredMessage(
            message = paste0(
    ...
    printColoredMessage(message = '-- Finding genes that are highly affected by  source(s) of unwanted variation:',
                        color = 'magenta',
                        verbose = verbose)
    printColoredMessage(message = 'Note, this step with be performed within each possible homogeneous biological groups.',
                        color = 'blue',
                        verbose = verbose)
    printColoredMessage(message = 'Creating all possible major homogeneous biological groups:',
                        color = 'blue',
                        verbose = verbose)
    ...
    if (length(continuous.uv) > 0) {
        ...
        printColoredMessage(
            message = paste0(length(continuous.uv), ' continuous ', sour, ' of unwanted variation:',
                             paste0(continuous.uv, collapse = ' & '), ' ', group, ' found in the uv.variables.'),
            color = 'blue',
            verbose = verbose
        )
        ...
        if (length(selected.bio.groups) > 0) {
            ...
            printColoredMessage(
                message = paste0(length(selected.bio.groups), ' homogeneous biological ', group, ' at least ',
        ...
        anova.genes.uv <- lapply(
            ...
            function(x) {
                ...
                if (sum(rowSums(bio.batch >= min.sample.for.aov) == l.gorup) > 0) {
                    ...
                    printColoredMessage(
                        message = paste0( sum(rowSums(bio.batch > min.sample.for.aov) == l.gorup), ' homogeneous biological ',
                            group, ' at least ', min.sample.for.aov,
                            ' (min.sample.for.aov) samples within individual batches of the ', x, ' variable.'),
                        color = 'blue',
                        verbose = verbose)
                } else {
                    printColoredMessage(
                        message = paste0('There are not homogeneous biological groups have at least ',
    ...
    printColoredMessage(message = '-- Step2: finding genes that are not highly affected by different sources of biological variation:',
                        color = 'magenta',
                        verbose = verbose)
    printColoredMessage(message = 'This step with be performed within each possible homogeneous unwanted groups.',
                        color = 'red',
                        verbose = verbose)
    ## step 2.1 create all possible homogeneous uv groups ####
    printColoredMessage(message = '### Creating all possible major groups with respect to sources of unwanted variation:',
                        color = 'magenta',
                        verbose = verbose)
    ...
    if (length(continuous.bio) > 0) {
        ...
        printColoredMessage(
            message = paste0(length(continuous.bio), ' continuous ', sour, ' of biological variation: ',
                paste0(continuous.bio, collapse = ' & '), ' ', group, ' found in the bio.variables.'),
            color = 'blue',
            verbose = verbose
        )
                    ...
                    if (max(covered.batches) == length(unique(se.obj[[x]]))) {
                        printColoredMessage(
                            message = paste0(
                                ...
                                'variation can cover all the groups of the ', x, ' variable.'),
                            color = 'white',
                            verbose = verbose)
                    } else {
                        printColoredMessage(
                            message = paste0(
                                'Please note that the homogeneous groups with respect to unwanted variation do not',
                                'cover all the groups of the ', x,' variable.'),
                            color = 'blue',
                            verbose = verbose)
                    }
                }
                ...
                all.anova <- lapply(
                    ...
                    function(i) {
                        ...
                        anova.gene.bio <- as.data.frame(
                            row_oneway_equalvar(x = data.to.use[, selected.samples],
                                                g = se.obj@colData[, x][selected.samples]))
                        set.seed(2233)
    ...
    if (ncg.selection.method %in% c('Prod', 'Sum', 'Average')) {
        ...
        ncg.selected <-
            ncg.selected[order(ncg.selected$stat.rank, decreasing = FALSE) , ]
        ncg.selected <-
            row.names(ncg.selected[1:round(nb.ncg / 100 * nrow(se.obj), digits = 0) , ])
        ncg.selected <- row.names(se.obj) %in% ncg.selected
    } else if (ncg.selection.method == 'noneOverlap') {
        printColoredMessage(message = 'A set of NCG is selected based on the noneOverlap approach.',
                            color = 'blue',
                            verbose = verbose)
        printColoredMessage(
            message = paste0(
        ...
        if(length(ncg.selected) > nb.ncg){
            ...
            pro.bar <- progress_estimated(round(c(top.rank.uv.genes/grid.nb), digits = 0) + 2)
        ...
        printColoredMessage(message = 'A set of NCG is selected based on the AbsNoneOverlap approach.',
                            color = 'blue',
                            verbose = verbose)
        printColoredMessage(
            message = paste0(
    ...
    if (assess.ncg) {
        ...
        printColoredMessage(message = 'Peform PCA on only selected genes as NCG.',
                            color = 'blue',
                            verbose = verbose)
        printColoredMessage(
            message = paste0( 'Then, explore the association of the first ', nb.pcs, '  with the ',
                paste0(c(all.variables, uv.variables), collapse = ' & '), ' variables.'),
            color = 'blue',
            verbose = verbose
        )
        if (apply.log) {
            temp.data <- log2(assay(se.obj, assay.name) + pseudo.count)
        } else temp.data <- assay(se.obj, assay.name)
    ...
    if (save.se.obj == TRUE) {
        ...
        printColoredMessage(message = 'The NCGs are saved to metadata of the SummarizedExperiment object.',
                            color = 'blue',
                            verbose = verbose)
        printColoredMessage(message = '------------The supervisedFindNcgPbPbio function finished.',
                            color = 'white',
                            verbose = verbose)
        return(se.obj)
    } else{
