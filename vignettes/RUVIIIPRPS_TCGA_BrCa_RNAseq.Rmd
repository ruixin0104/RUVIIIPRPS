...
### Un supervised 
```{r}
brca.se.obj <- findNcgsUnSupervised(
    se.obj = brca.se.obj, 
    assay.name = 'RawCount', 
     ncg.selection.method ='AbsNoneOverlap',
    grid.nb = 1,
    top.rank.bio.genes = 80,
    top.rank.uv.genes = 10,
    uv.variables = c('library.size', 'batch.FcCh', 'tumour.purity.estimate'),
    variables.to.assess.ncg = c('PAM50','library.size', 'batch.FcCh', 'tumour.purity.estimate'))
brca.se.obj@metadata$NCG$un.supervised$
```
...
# Estimate possible unknown batch effects

```{r}
raw.counts <- log2(assay(brca.se.obj, 'unstranded') + 1)
mm <- apply(
    raw.counts, 
    1, function(x) cor.test(x, brca.se.obj@metadata$UknownUV$unstranded$BrCa_Batches$input.data)[[4]])
ncg <- names(sort(mm, decreasing = T)[1:200])

gene.an <- rowData(brca.se.obj)
ncg <- gene.an$micorarray.hk.genes

brca.se.obj <- RUVIIIPRPS::identifyUnknownUV(
    se.obj = brca.se.obj[ncg , ], 
    assay.name = 'unstranded', approach = 'pca',  nb.pcs = 3, 
    max.samples.per.batch = .4, 
    order.batches = F,
    remove.current.estimates = TRUE,
    output.name = 'BrCa_Batches')
temp <- table(brca.se.obj@metadata$UknownUV$unstranded$BrCa_Batches$batches,
      brca.se.obj$batch.years)
ComplexHeatmap::Heatmap(
    ...
    name = 'Frequency')
brca.se.obj$unknown.batch <- brca.se.obj@metadata$UknownUV$unstranded$BrCa_Batches$batches
```
